...
def ingest_sensor_observation_payload(graph: FarmGraph, data: dict):
    ...
    try:
        # Start transaction explicitly to lock the state
        conn.execute("BEGIN")
        ...
        if node_status != "accepted":
            ...
            graph.storage.add_quarantined_observation(
                ...
                commit=False # Part of transaction
            )
            conn.execute("COMMIT")
        ...
        if not graph.get_node(node_id):
            ...
            graph.add_node(sensor, commit=False)
            if sensor.zone_id:
                graph.add_edge(node_id, "DEPLOYED_IN", sensor.zone_id, commit=False)
            elif sensor.field_id:
                graph.add_edge(node_id, "DEPLOYED_IN", sensor.field_id, commit=False)

        if not graph.get_node(meas_id):
            measurement = Measurement(id=meas_id, farm_id=data["farm_id"], layer=data["layer"])
            graph.add_node(measurement, commit=False)
            graph.add_edge(meas_id, "INFORMS", f"ontology:{data['layer']}", commit=False)

        graph.add_edge(node_id, "MEASURES", meas_id, commit=False)
        graph.add_node(obs, commit=False)
        graph.add_edge(obs.id, "PRODUCES", meas_id, commit=False)
